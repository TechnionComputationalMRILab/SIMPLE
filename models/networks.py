...
from .unet_ddm import Unet, WBlock
...
def get_norm_layer(norm_type='instance', net='D'):
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[], **kwargs):
    ...
    """
    net = None
    norm_layer = get_norm_layer(norm_type=norm, net='G')
    ...
    if netG == 'resnet_9blocks':
        ...
        net = Unet(
            ...
            resnet_block_groups = kwargs['resnet_block_groups'],
            learned_sinusoidal_cond = kwargs['learned_sinusoidal_cond'],
            random_fourier_features = kwargs['random_fourier_features'],
            learned_sinusoidal_dim = kwargs['learned_sinusoidal_dim'],
            time_dim_mult = kwargs['time_dim_mult']
        )
    else:
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, gpu_ids=[], mask_size=256, s1=64, s2=32):
