...
class DiscPool(Dataset):
    ...
    def __init__(self, opt, device, dataset, isTrain=True, disc_out_size=30):
        ...
        if isTrain:
            ...
            self.disc_out = torch.rand((self.dataset_len, 1, disc_out_size, disc_out_size), dtype=torch.float32)
            # self.disc_out = torch.rand((self.dataset_len, 1, 1, disc_out_size, disc_out_size), dtype=torch.float32) #TODO: need to understand why there is missing 1 dimension
        else:
            # At the end, the discriminator is expected to be near its maximum entropy state (D_i = 1/2)
            self.disc_out = 0.5 + 0.001 * torch.randn((self.dataset_len, 1, disc_out_size, disc_out_size),
                                                      dtype=torch.float32)
